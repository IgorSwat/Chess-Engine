...
Button::Button(float size, sf::Texture* contentTexture) : background(size)
{
...
void Button::scaleContent(sf::Texture* contentTexture)
{
    unsigned int textureSize = std::max(contentTexture->getSize().x, contentTexture->getSize().y);
    float scaleFactor = 1.6 * background.getRadius() / textureSize;
...
void Button::setContentTexture(sf::Texture* contentTexture)
{
