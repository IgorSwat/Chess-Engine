...
Navbar::Navbar(const sf::Vector2f& initialPos, const sf::Vector2f& initialSize, std::map<std::string, sf::Texture*>& txts)
    : leftCorner(initialPos), size(initialSize), textures(txts)
{
    for (int i = 0; i < 3; i++)
    {
...
void Navbar::setMove(const Move& move, int moveID)
{
    ...
    if (moves[moveID]->color == COLOR::WHITE)
        textureName += "W";
    else
        textureName += "B";
