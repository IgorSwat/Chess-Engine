...
public:
	...
	void setKey(const Vector2i& dir);
	void setKeys(const initializer_list<Vector2i>& dirs) { for (const Vector2i& dir : dirs) setKey(dir); }
	void add(const Vector2i& dir, const Move2& move) { moves[keyMapping[KEY_STARTING_POINT + directionHash(dir)]].push_back(move); size++; }
	void erase(const Vector2i& dir) {
	...
	const vector<Move2>& getMoves(const Vector2i& dir) { return moves[keyMapping[KEY_STARTING_POINT + directionHash(dir)]]; }
	...
	static int directionHash(const Vector2i& dir) { return dir.x * 10 + dir.y; }
};
