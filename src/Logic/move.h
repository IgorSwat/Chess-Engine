...
public:
    ...
    const COLOR color;
    const int pieceID;
    const sf::Vector2i targetPos;
    const int moveNumber;
    const PieceType promotionFlag;
    const MoveMask specialFlag;
    const float evaluation;
    Move() : piece(PieceType::KING), color(COLOR::WHITE), pieceID(0), targetPos(sf::Vector2i(0, 0)), moveNumber(1),
        promotionFlag(PieceType::PAWN), specialFlag(MoveType::COMMON), evaluation(0.f) {}
    Move(PieceType movingPiece, COLOR c, int id, const sf::Vector2i& pos, int moveno, PieceType promotionf,
    ...
    Move(Piece* movingPiece, const sf::Vector2i& pos, int moveno, PieceType promotionf,
         MoveMask special = MoveMask(MoveType::COMMON), float eval = 0.f)
...
public:
    ...
    const MoveMask specialFlag;
    const PieceType promotionFlag;
    Move2(int id, PieceType type, const sf::Vector2i& pos, MoveMask special = MoveMask(MoveType::COMMON),
