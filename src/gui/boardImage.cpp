...
namespace GUI {
    ...
    void BoardImage::loadPosition(const BoardConfig* board)
    {
        ...
        if (board->isInCheck(board->movingSide())) {
            Square sq = board->kingPosition(board->movingSide());
    ...
    std::pair<Move, bool> BoardImage::update(const sf::Event& event, sf::Vector2i mousePos, const BoardConfig* board)
    {
        // Case 1 - no promotion awaiting, just moving pieces
        if (activePromotionBar == nullptr) {
            // Mouse click, try to select some piece
            if (event.type == sf::Event::MouseButtonPressed && event.key.code == sf::Mouse::Left && !selectedPiece) {
                ...
                if (selected != pieces.end() && color_of((*selected)->representedPiece()) == board->movingSide()) {
            ...
            // Mouse button released, try to place selected piece (if exists) on new square
                ...
                if (from != to) {
                    ...
                    return { MoveGeneration::create_pseudo_move(*board, from, to), false };
                }
                else {
            ...
            if (promoteTo != NULL_TYPE) {
                activePromotionBar = nullptr;
                moveMarkerState = true;
                Movemask promotionMask = promoteTo == QUEEN ? QUEEN_PROMOTION_FLAG :
                                         promoteTo == ROOK ? ROOK_PROMOTION_FLAG :
                                         promoteTo == KNIGHT ? KNIGHT_PROMOTION_FLAG : BISHOP_PROMOTION_FLAG;
    ...
    {
        return sf::Vector2f(
            file_of(sq) * tileSize,         // x
            (7 - rank_of(sq)) * tileSize    // y
        );
    }
    ...
    {
        return make_square(
            std::clamp(7 - int(coords.y / tileSize), 0, 7),
            std::clamp(int(coords.x / tileSize), 0, 7)
        );
    }
